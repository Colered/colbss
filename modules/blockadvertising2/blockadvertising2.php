...
class BlockAdvertising2 extends Module
{
	...
	protected function initialize()
	{
		...
		if ((Shop::getContext() == Shop::CONTEXT_GROUP  || Shop::getContext() == Shop::CONTEXT_SHOP)
			&& file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'-g'.$this->context->shop->getContextShopGroupID().'.'.Configuration::get('BLOCKADVERT2_IMG_EXT')))
			$this->adv_imgname .= '-g'.$this->context->shop->getContextShopGroupID();
		if (Shop::getContext() == Shop::CONTEXT_SHOP
			&& file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'-s'.$this->context->shop->getContextShopID().'.'.Configuration::get('BLOCKADVERT2_IMG_EXT')))
			$this->adv_imgname .= '-s'.$this->context->shop->getContextShopID();

		$this->adv_img = Tools::getMediaServer($this->name)._MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT2_IMG_EXT');
		$this->adv_link = htmlentities(Configuration::get('BLOCKADVERT2_LINK'), ENT_QUOTES, 'UTF-8');
		$this->adv_title = htmlentities(Configuration::get('BLOCKADVERT2_TITLE'), ENT_QUOTES, 'UTF-8');
	}
	
	public function install()
	{
		Configuration::updateGlobalValue('BLOCKADVERT2_LINK', 'http://www.prestashop.com/');
		Configuration::updateGlobalValue('BLOCKADVERT2_TITLE', 'PrestaShop');
		...
		foreach (scandir(_PS_MODULE_DIR_.$this->name) as $file)
			if (in_array($file, array('advertising.jpg', 'advertising.gif', 'advertising.png')))
				Configuration::updateGlobalValue('BLOCKADVERT2_IMG_EXT', substr($file, strrpos($file, '.') + 1));
	...
	{
		Configuration::deleteByName('BLOCKADVERT2_LINK');
		Configuration::deleteByName('BLOCKADVERT2_TITLE');
		Configuration::deleteByName('BLOCKADVERT2_IMG_EXT');
	...
	private function _deleteCurrentImg()
	{
		...
		if ($this->adv_imgname != 'advertising' && file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT2_IMG_EXT')))
			unlink(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT2_IMG_EXT'));
		
		// Update the extension to the global value or the shop group value if available
		Configuration::deleteFromContext('BLOCKADVERT2_IMG_EXT');
		Configuration::updateValue('BLOCKADVERT2_IMG_EXT', Configuration::get('BLOCKADVERT2_IMG_EXT'));
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAdvConf'))
		{
			if (isset($_FILES['adv_img']) && isset($_FILES['adv_img']['tmp_name']) && !empty($_FILES['adv_img']['tmp_name']))
			{
				...
				{
					Configuration::updateValue('BLOCKADVERT2_IMG_EXT', substr($_FILES['adv_img']['name'], strrpos($_FILES['adv_img']['name'], '.') + 1));
					...
					if (!move_uploaded_file($_FILES['adv_img']['tmp_name'], _PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT2_IMG_EXT')))
			...
			if ($link = Tools::getValue('adv_link'))
				Configuration::updateValue('BLOCKADVERT2_LINK', $link);
			elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP)
				Configuration::deleteFromContext('BLOCKADVERT2_LINK');
			...
			if ($title = Tools::getValue('adv_title'))
				Configuration::updateValue('BLOCKADVERT2_TITLE', $title);
			elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP)
				Configuration::deleteFromContext('BLOCKADVERT2_TITLE');
	...
	public function getContent()
	{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" enctype="multipart/form-data">
			<fieldset>
				<legend>'.$this->l('Advertising 2 block configuration').'</legend>';
		if ($this->adv_img)
		{
			$output .= '
