...
include_once(_PS_MODULE_DIR_.'tmrightbanner/rightBanner.php');

class TmRightBanner extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'tmrightbanner';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Right Banner');
		$this->description = $this->l('Adds a banners for Right Column.');
	}
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 1; ++$i)
		{
			$slide = new rightBanner();
			$slide->position = $i;
			$slide->active = 1;
			foreach ($languages as $language)
			{
				$slide->title[$language['id_lang']] = 'RightBanner '.$i;
				$slide->url[$language['id_lang']] = 'http://www.templatemela.com/';
				$slide->image[$language['id_lang']] = 'right-banner-'.$i.'.jpg';
			}
			$slide->add();
		}
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmrightbanner` (
				`id_tmrightbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_tmrightbanner_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmrightbanner_slides` (
			  `id_tmrightbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_tmrightbanner_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmrightbanner_slides_lang` (
			  `id_tmrightbanner_slides` int(10) unsigned NOT NULL,
			  `id_lang` int(10) unsigned NOT NULL,
			  `title` varchar(255) NOT NULL,
			  `url` varchar(255) NOT NULL,
			  `image` varchar(255) NOT NULL,
			  PRIMARY KEY (`id_tmrightbanner_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new rightBanner($slide['id_slide']);
			$to_del->delete();
		}
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmrightbanner`, `'._DB_PREFIX_.'tmrightbanner_slides`, `'._DB_PREFIX_.'tmrightbanner_slides_lang`;
		');
	}
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new rightBanner((int)Tools::getValue('id_slide'));
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<input type="hidden" name="has_picture" value="1" /><img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" alt="rightbanner"/>';
			$this->_html .= '</div>';
		}
	...
	private function _postProcess()
	{
		...
		{
			$slide = new rightBanner((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			if (!$res)
	...
	private function _prepareHook()
	{
		...
		$this->smarty->assign('tmrightbanner_slides', $slides);
	...
	public function hookRightColumn()
	{
		...
		return $this->display(__FILE__, 'tmrightbanner.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
		INSERT IGNORE INTO '._DB_PREFIX_.'tmrightbanner (id_tmrightbanner_slides, id_shop)
		SELECT id_tmrightbanner_slides, '.(int)$params['new_id_shop'].'
		FROM '._DB_PREFIX_.'tmrightbanner
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				FROM `'._DB_PREFIX_.'tmrightbanner_slides` hss, `'._DB_PREFIX_.'tmrightbanner` hs
				WHERE hss.`id_tmrightbanner_slides` = hs.`id_tmrightbanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_tmrightbanner_slides` as id_slide,
					   hssl.`image`,
					   hss.`position`,
					   hss.`active`,
					   hssl.`title`,
					   hssl.`url`
			FROM '._DB_PREFIX_.'tmrightbanner hs
			LEFT JOIN '._DB_PREFIX_.'tmrightbanner_slides hss ON (hs.id_tmrightbanner_slides = hss.id_tmrightbanner_slides)
			LEFT JOIN '._DB_PREFIX_.'tmrightbanner_slides_lang hssl ON (hss.id_tmrightbanner_slides = hssl.id_tmrightbanner_slides)
	...
	{
		$req = 'SELECT hs.`id_tmrightbanner_slides` as id_slide
				FROM `'._DB_PREFIX_.'tmrightbanner` hs
				WHERE hs.`id_tmrightbanner_slides` = '.(int)$id_slide;
