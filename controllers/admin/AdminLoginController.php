		...
		if (!count($this->errors))
		{
			...
			else
			{
				...
				if (isset($_POST['redirect']) && Validate::isControllerName($_POST['redirect'])){
					...
					$sql = 'SELECT view from ps_access where id_profile ='.$this->context->cookie->profile.' and id_tab ='.$this->context->employee->default_tab;
					$res = Db::getInstance()->getValue($sql);
					if($res == 1)
						$url = $this->context->link->getAdminLink($_POST['redirect']);
					else
						$url = $this->context->link->getAdminLink('AdminDashboard');//die;
				}
				else
				{
					...
					$res = Db::getInstance()->getValue($sql);
					if($res == 1)				
					 $url = $this->context->link->getAdminLink($tab->class_name);//die;
					else
						$url = $this->context->link->getAdminLink('AdminDashboard');//die;
				}
