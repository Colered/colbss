...
class OrderOpcControllerCore extends ParentOrderController
{
	...
	{
		parent::init();
		include_once('../'.__PS_BASE_URI__ .'override/classes/NcfOrder.php');
		if ($this->nbProducts)
					...
					switch (Tools::getValue('method'))
					{
						...
						case 'updateNcfData':
							...
							if (Tools::isSubmit('perName'))
							{
								...
								$this->_updateNcfData($txtperName, $txtrncId, $txtidphone, $txtidaddress, $txtncftype1);
								if (count($this->errors))
									die('{"hasError" : true, "errors" : ["'.implode('\',\'', $this->errors).'"]}');
								die(true);
							}
	...
	{
		$oldNcfData = NcfOrder::getNcfdataByCartId((int)($this->context->cart->id));
		$this->context->smarty->assign(array(
