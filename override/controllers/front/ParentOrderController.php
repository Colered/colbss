...
class ParentOrderControllerCore extends FrontController
{
	...
	protected function _updateNcfData($txtperName, $txtrncId, $txtidphone, $txtidaddress, $txtncftype1)
	{
		...
		if ($txtncftype1)
		{
			if ($oldNcfData = NcfOrder::getNcfdataByCartId((int)($this->context->cart->id)))
			{
				...
				$ncfOrder->rnc_name = $txtperName;
				$ncfOrder->rnc_id = $txtrncId;
				$ncfOrder->rnc_phone = $txtidphone;
				$ncfOrder->rnc_address = $txtidaddress;
				$ncfOrder->rnc_type = $txtncftype1;
				$ncfOrder->update();
			}
			else
			{
		...
		else
		{
			if ($oldNcfData = NcfOrder::getNcfdataByCartId((int)($this->context->cart->id)))
			{
				$ncfOrder = new NcfOrder($oldNcfData['id']);
				$ncfOrder->delete();
			}
		}
		return true;
