...
class AuthController extends AuthControllerCore
{
	...
	{          
				...
				$sql_alias =Db::getInstance()->ExecuteS('SELECT alias,id_country FROM '._DB_PREFIX_.'address WHERE id_customer='.$this->context->cookie->id_customer.'');
				if(count($sql_alias)>0){
				    for($i=0;$i<count($sql_alias);$i++){
					    $alias[]=$sql_alias[$i]['alias'];
				    }
				}
				if(in_array("School address", $alias)){ 
				echo "UPDATE";
