...
class PreCartControllerCore extends FrontController
{
	public $php_self = 'precart';
	...
	public function postProcess()
	{
		...
		if ($this->context->cookie->exists() && !$this->errors && !$this->isTokenValid())
		{
			...
			if(isset($_POST['courseIdArr']) && count(array_filter($_POST['courseIdArr']))>0){
				$courseIds = implode(',' , array_filter($_POST['courseIdArr']));
				$schoolid = $_POST['sid'];
				$qry = "SELECT psp.id_product, fcb.quantity, fcb.id, fcb.course_id, fcb.school_id, fcb.book_reference_no FROM fed_course_books as fcb LEFT JOIN ps_product as psp ON fcb.book_reference_no = psp.reference WHERE fcb.school_id =$schoolid and fcb.course_id IN ($courseIds)";
				$productsDatas = Db::getInstance()->ExecuteS($qry);
				foreach($productsDatas as $productsData){
					if($productsData['id_product']<>''){
						//get product detail using product Id
						$product = new Product( (int)$productsData['id_product']);
						$this->id_product = $productsData['id_product'];
						//$this->id_product_attribute = (int)Tools::getValue('id_product_attribute', Tools::getValue('ipa'));
						//$this->customization_id = (int)Tools::getValue('id_customization');
						$this->qty = (int)($productsData['quantity']);
				...
				//redirect user to bookstore if no grade is selected
