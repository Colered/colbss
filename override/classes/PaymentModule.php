		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					//specify order id for ncf module after payment data
					//save school reference ID from session
					@session_start();
					$schoolId = "";
					if(isset($_SESSION['sid']) && $_SESSION['sid'] !=''){
						$schoolId = $_SESSION['sid'];
					}
					if ($oldNcfData = NcfOrder::getNcfdataByCartId((int)($this->context->cart->id)))
					{
						
						//generate fiscal reciept no
						$fiscal_receipt_num_p1 = 'A0100100101';
						//select max allocated fiscal no p2 in series A0100100101
						$sql1 = "SELECT MAX(fiscal_receipt_num_p2) as maxfsc FROM ps_fed_order_nfc_details WHERE  `fiscal_receipt_num_p1` =  'A0100100101'";
						if ($row = Db::getInstance()->getRow($sql1)){
								if(is_null($row['maxfsc'])){
										$fiscal_receipt_num_p2 = '000000005';
								}else{
										$fiscal_receipt_num_p2 = (($row['maxfsc']) + 1);
								}
						}else{
								$fiscal_receipt_num_p2 = '000000005';
						}
						$fiscal_receipt_num_p2; 
						$ncfOrder = new NcfOrder((int)($oldNcfData['id']));
						$ncfOrder->order_id = (int)$order->id;
						$ncfOrder->school_id = $schoolId;
						$ncfOrder->fiscal_receipt_num_p1 = 'A0100100101';
						$ncfOrder->fiscal_receipt_num_p2 = str_pad($fiscal_receipt_num_p2, 9, '0', STR_PAD_LEFT);
						$ncfOrder->update();
					}else{
						//generate fiscal reciept no
						$fiscal_receipt_num_p1 = 'A0100100102';
						//select max allocated fiscal no p2 in series A0100100102
						$sql2 = "SELECT MAX(fiscal_receipt_num_p2) as maxfsc FROM ps_fed_order_nfc_details WHERE  `fiscal_receipt_num_p1` =  'A0100100102'";
						if ($row = Db::getInstance()->getRow($sql2)){
								if(is_null($row['maxfsc'])){
										$fiscal_receipt_num_p2 = '00000001';
								}else{
										$fiscal_receipt_num_p2 = (($row['maxfsc']) + 1);
								}
						}else{
						...
						$ncfOrder = new NcfOrder();
						$ncfOrder->order_id = (int)$order->id;
						$ncfOrder->school_id = $schoolId;
						$ncfOrder->id_cart = (int)($this->context->cart->id);
						$ncfOrder->id_customer = (int)($this->context->cart->id_customer);
						$ncfOrder->fiscal_receipt_num_p1 = 'A0100100102';
						$ncfOrder->fiscal_receipt_num_p2 = str_pad($fiscal_receipt_num_p2, 9, '0', STR_PAD_LEFT);
						$ncfOrder->add();
					}
					//end
