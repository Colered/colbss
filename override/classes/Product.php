...
class Product extends ProductCore
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'reference' => 					array('type' => self::TYPE_STRING, 'validate' => 'isReference', 'required' => true, 'size' => 32),
	...
	public static function getReferenceById($reference, $product_id)
	{
		$query = new DbQuery();
		$query->select('p.reference');
		$query->from('product', 'p');
		$query->where('p.reference = \''.$reference.'\'');
		$query->where('p.id_product != '.$product_id);	
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($query);
	}	
}